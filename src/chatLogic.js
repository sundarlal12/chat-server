 ...
 * own order_id generator exactly (api/v1/api/add-fund.php's $genRef) - the
 * fallback `tr` used only when createDepositOrder couldn't open a real
 * order (see maybeAutoReplyToDepositGreeting).
 */
function genFallbackOrderRef() {
  const now = new Date();
  const pad = (n) => String(n).padStart(2, '0');
  const stamp = `${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}`;
  return stamp + String(Math.floor(Math.random() * 1e9)).padStart(9, '0');
}
...
async function maybeAutoReplyToDepositGreeting(ticketBeforeThisMessage, message, customerToken) {
  ...
  const tr = order ? order.orderId : genFallbackOrderRef();
  // No `tn` (UPI "transaction note") - PG/gas_client.php's reconciliation
  // lookup only ever sends `orderId` (this `tr`) to the GAS status API,
  // never tn, so it's pure dead weight here: extra characters that push
  // the QR into a denser encoding for zero functional benefit. Dropping
  // it alone takes this from a 45x45-module code down to 41x41.
  ...
  // margin:4 is the QR spec's minimum quiet zone (modules, not pixels) -
  // margin:1 (an earlier attempt) left almost no white border, which is
  // exactly why the code looked like it ran edge-to-edge/"scattered" in
  // the chat bubble. width:130 (down from 220) per operator follow-up
  // report that 220 still rendered too large in the app's chat image
  // bubble - at 41x41 modules + 8 quiet-zone modules this is ~2.7px/module,
  // still within camera-scannable range though with less margin for a
  // blurry/glare-y screen-to-screen scan than the larger size had.
  const qrPng = await QRCode.toBuffer(upiIntent, { type: 'png', width: 130, margin: 4, errorCorrectionLevel: 'M' });
