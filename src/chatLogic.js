...
async function maybeAutoReplyToDepositGreeting(ticketBeforeThisMessage, message, customerToken) {
  ...
  // the previous margin:1 left almost no white border, which is exactly
  // why the code looked like it ran edge-to-edge/"scattered" in the chat
  // bubble screenshot reported back. 220px keeps each module comfortably
  // scannable even for a screen-to-screen scan (harder than paper) now
  // that the quiet zone is eating more of the frame.
  const qrPng = await QRCode.toBuffer(upiIntent, { type: 'png', width: 220, margin: 4, errorCorrectionLevel: 'M' });
