 ...
 */
// Deliberately a strict, narrow allow-list (jpg/png/gif images, mp4 video,
// mp3 audio, webm/ogg voice notes) rather than covering every real-world
// device format - every entry here must have a verifiable magic-byte check
// in sniffKind() below, since that's what actually gates an upload now
// (see makeUploadHandler in routes/attachments.js), not the client-claimed
// Content-Type/filename alone. A wider allow-list without a matching sniff
// signature would just create formats that pass classify() but can never
// pass verification - each entry needs both.
const ALLOWED = [
...
/**
 * Classifies an upload by mimetype first, falling back to file extension
 * for clients that send a generic mimetype (e.g. application/octet-stream).
 * Returns null if the type isn't in the allow-list.
 *
 * This alone is NOT a security boundary - both mimetype and filename come
 * straight from the client's multipart headers, so a malicious upload can
 * claim anything here. makeUploadHandler (routes/attachments.js) always
 * follows this with sniffKind() against the actual received bytes and
 * rejects anything that doesn't verify, specifically so a file can't get
 * treated/served as an image or audio file by lying about its
 * Content-Type/extension alone (the classic file-upload-to-XSS path, since
 * whatever mime type gets stored here is also what chat-attachment/:oid
 * later serves the file back as).
 */
...
const ALLOWED_DESCRIPTION = 'images (jpg, png, gif), video (mp4), audio (mp3), or a recorded voice note (webm, ogg)';

/**
 * Magic-byte sniff of the actual received content - the real gate on what
 * gets accepted (see makeUploadHandler in routes/attachments.js), not just
 * a fallback for the inline-base64 socket path (chatLogic.js's
 * resolveInlineAttachment) it was originally written for. Every ALLOWED
 * entry has a corresponding check here; nothing is accepted on
 * mimetype/extension claims alone.
 */
